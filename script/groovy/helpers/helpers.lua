...
---@return string
function GLab.HandleLocalisedText(str, default, auto)
...
function GLab.TriggerPopup(popup_key, text, use_two_buttons, button_one_callback, button_two_callback, creation_callback, opt_parent)
    -- verify shit is alright
    if not is_string(popup_key) then
        GLab.Error("trigger_popup() called, but the key passed is not a string!")
        return false
    end
    ...
    if not is_string(text) then
        GLab.Error("trigger_popup() called, but the text passed is not a string!")
        return false
    end
    ...
    if not is_boolean(use_two_buttons) then
        GLab.Error("trigger_popup() called, but the two_buttons arg passed is not a boolean!")
        return false
    end
    ...
    core:get_tm():real_callback(function()
        ...
        if creation_callback and is_function(creation_callback) then
            ...
            function(context)
				...
				local ok, er = pcall(function() 
                    ...
                    if context.string == "button_tick" then
                        if button_one_callback and is_function(button_one_callback) then
                            button_one_callback()
                        end
                    else
                        if button_two_callback and is_function(button_two_callback) then
                            button_two_callback()
                        end
                    end
